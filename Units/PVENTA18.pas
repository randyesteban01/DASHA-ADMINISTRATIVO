...
// --- Copias tfa_copias (Paso 1: helpers, aun no se usan) ---
...
begin
  ...
  begin
    ...
    Open;
    if not IsEmpty then
      frmFactura.copias := FieldByName('tfa_copias').AsInteger;
  end;
end;
...
var
  Lbl: TQRLabel;
begin
  if Reporte.FindComponent('lbNumCopia') = nil then
    Exit;

  Lbl := TQRLabel(Reporte.FindComponent('lbNumCopia'));
  if (TotalCopias > 1) and (NumCopia > 1) then
  begin
    Lbl.Caption := Format('Copia %d de %d', [NumCopia, TotalCopias]);
    Lbl.Enabled := True;
  end
  else
  begin
...
begin
  ...
  while a <= vTotal do
  begin
    EstablecerEtiquetaCopia(Reporte, a, vTotal);
    Reporte.Print;
    Inc(a);
  end;
end;
...
begin
  ...
  while a <= vTotal do
  begin
    if (vTotal > 1) and (a > 1) then
      sCopia := Format('Copia %d de %d', [a, vTotal])
    else
      sCopia := '';
    try
...
if (RG_BuscaDet.ItemIndex = 2) and (Buscando = False) and (Not realizado) and (DesactivarCargaProductos = False) then begin
    ...
    begin
      ...
      if  MessageDlg('Ya este producto existe en la lista, desea continuar?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
        ...
        if not dm.Query1.FieldByName('pro_comision').IsNull then begin
         ...
         if ((FormatoImp = 2) OR FileExists('.\Transporte.Txt')) then
                begin
                  ...
                  {      if dm.QParametrospar_mostrarfacturadolares.Value = true then
                        ...
                        ImprimirQuickRepCopias(RFactura); // Paso 3
                        RFactura.Destroy;
                   end;
                      end
...
begin
  codigoabre := Trim(DM.QParametrosPAR_CODIGO_ABRE_CAJA.Value);
      CargarCopiasTipoFactura; // Paso 2
...
if VarIsNull(QFacturaFAC_TASA.Value) or (QFacturaFAC_TASA.Value = 0) then
  ...
  QFacturaFAC_TOTALUS.Value :=
    QFacturaFAC_TOTAL.Value / QFacturaFAC_TASA.Value;
    end
else
begin
