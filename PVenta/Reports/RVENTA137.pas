...
type                                       
  TRFacturaConstruccion = class(TQuickRep)
    QRBand1: TQRBand;
    lbFactura: TQRLabel;
    QRLabel4: TQRLabel;
    QRLabel5: TQRLabel;
    ...
    lblMontoGravado: TQRLabel;
    lblItbis: TQRLabel;
    ltlTotal: TQRLabel;
    dtaServicio: TQRDBText;
    ...
    QFacturaFAC_PROYECTO: TFloatField;
    QFacturaFAC_SERVICIOS: TFloatField;
    dtaCostoProyecto: TQRDBText;
...
var
  RFacturaContruccion: TRFacturaConstruccion;
...
procedure TRFacturaConstruccion.GenerarCodigoQR(const Texto: string; Imagen: TQRImage);
var
...
procedure TRFacturaConstruccion.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  a : integer;
begin
...
procedure TRFacturaConstruccion.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
var
  a : integer;
  cambiar : TQRLabel;
  TieneServicioConstruccion: Boolean;
begin
    lbNumCopia.Caption := '';
  ...
  if not QFacturaFAC_MENSAJE2.IsNull then
    ...
    TieneServicioConstruccion := False;

  dm.Query1.Close;
  dm.Query1.SQL.Clear;
  dm.Query1.SQL.Add('select top 1 1 as existe');
  dm.Query1.SQL.Add('from det_factura d');
  dm.Query1.SQL.Add('inner join productos p on p.emp_codigo = d.emp_codigo and p.pro_codigo = d.pro_codigo');
  dm.Query1.SQL.Add('where d.emp_codigo = :emp');
  dm.Query1.SQL.Add('and d.suc_codigo = :suc');
  dm.Query1.SQL.Add('and d.fac_forma = :for');
  dm.Query1.SQL.Add('and d.tfa_codigo = :tfa');
  dm.Query1.SQL.Add('and d.fac_numero = :num');
  dm.Query1.SQL.Add('and isnull(p.with_servicio_construccion, 0) = 1');
  ...
  TieneServicioConstruccion := not dm.Query1.IsEmpty;

  if TieneServicioConstruccion then
  begin
    lblSubtotal.Caption:='COSTO PROY';
    lblMontoGravado.Caption:='MONTO GRAVADO';
    dtaServicio.DataField:= 'FAC_SERVICIOS';
    dtaCostoProyecto.DataField:='FAC_PROYECTO';
    lblitbis.Caption := 'NORMA 07-2007 ITBIS' ;
    lblDescripcion.Caption:='DESCRIPCION DE PARTIDAS';
    detCantidad.Visible:=False;
    detCantidad.DataField:='';
    lblCantidad.Caption:=''
  ...
  begin
    ...
    lblMontoGravado.Caption:='DESC';
    dtaServicio.DataField:= 'TOTALDESC';
    dtaCostoProyecto.DataField:='SUBTOTAL';
    lblitbis.Caption := 'ITBIS';
    lblDescripcion.Caption:='DESCRIPCION' ;
    detCantidad.Visible:=True;
  ...
  if Self.QRPrinter.PageNumber = Self.QRPrinter.PageCount then
  begin
...
if  dm.QParametrosUsa_FacturacionElectronica.Value  then
   ...
   if (not QFacturaAceptadoDGII.Value) and (not dm.QParametrosintegracion_luganis.AsBoolean) then
...
procedure TRFacturaConstruccion.QDetalleCalcFields(DataSet: TDataSet);
var
...
procedure TRFacturaConstruccion.ChildBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  t : string;
begin
...
procedure TRFacturaConstruccion.QFacturaCalcFields(DataSet: TDataSet);
var
  Fecha: TDateTime;
  FechaStr: string;
begin
...
procedure TRFacturaConstruccion.CorregirError;
var
  mov : string;
begin
  dm.Query1.Close;
...
procedure TRFacturaConstruccion.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  Linea := 1;
end;

procedure TRFacturaConstruccion.QRBand3AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  Linea := Linea + 1;
...
procedure TRFacturaConstruccion.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure TRFacturaConstruccion.ChildBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  dm.Query1.Close;
  dm.Query1.SQL.Clear;
