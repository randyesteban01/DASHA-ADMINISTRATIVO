...
uses PVENTA33, PVENTA34, RVENTA00, PVENTA36, SIGMA01, SIGMA00, PVENTA117,
  PVENTA121, PVENTA186, PVENTA15, PVENTA138, StrUtils, PVENTA253,
  DateUtils;
...
begin
      QParametros.Close;
      QParametros.Parameters.ParamByName('emp_codigo').Value := dm.vp_cia;
      QParametros.Open;
...
begin
  ...
  begin
    ...
    begin
      ...
      dm.Query1.sql.add('pro_roriginal, pro_rfabric, pro_itbis, pro_escala, pro_montoitbis, pro_comision');
      ...
      begin
        ...
        frmSelVendPorc.QVendedores.Parameters.ParamByName('EMP').Value := QPedidosEMP_CODIGO.Value;
...
begin
  ...
  if lbVendedor.Visible = true then
...
begin
     if QDetallePRO_CODIGO.Text <> '' then begin
      ...
      QMsnExistencia.Open;
        ...
        if QParametrospar_comision_vend.Value =  true then begin
        With qBuscVend do begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT RTRIM(ven_nombre) Vendedor');
        SQL.Add('FROM Vendedores');
        SQL.Add('where ven_codigo = '+IntToStr(QDetalleven_codigo.Value));
        Open;
        if IsEmpty then lbVendedor.Visible := False else begin
        lbVendedor.Visible := True;
        lbVendedor.Caption := 'Vendedor: '+fieldbyname('Vendedor').Text;
        end;
        end;
        end;
        end
...
procedure TfrmPedidosCli.QParametrosAfterOpen(DataSet: TDataSet);
begin
...
procedure TfrmPedidosCli.btBuscaVendPorcClick(Sender: TObject);
var
