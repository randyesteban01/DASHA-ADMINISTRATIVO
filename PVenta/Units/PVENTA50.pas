...
type
  TfrmConsDev = class(TForm)
    ...
    QDevemp_rnc: TStringField;
    QDevcli_rnc: TStringField;
    QDevEnviado_DGII: TBooleanField;
    QDevError_DGII: TBooleanField;
    cbStatusDGII: TRadioGroup;
    QDevAceptadoDGII: TBooleanField;
    QDeveNCF: TStringField;
    QDevmotivo_dgi: TIntegerField;
...
uses RVENTA08, SIGMA01, SIGMA00,FacturacionElectronicaDGII_TLB,ComObj;

{$R *.dfm}

function TfrmConsDev.ValidarENCFDisponible(
  AEmp: Integer; ATipo: Integer;
  out AMsg: string;
...
begin
  ...
  begin
   ...
   // DBGrid1.Columns[5].FieldName := 'eNCF';
    DBGrid1.Columns[5].Title.Caption :='eNCF';
  end;
...
begin
  ...
  begin
   ...
   if cbStatusDGII.ItemIndex = 1 then
     QDev.sql.add('and AceptadoDGII = 1')
  else if cbStatusDGII.ItemIndex = 2 then
     QDev.sql.add('and Enviado_DGII =1 and  Error_DGII=1 ')
  else if cbStatusDGII.ItemIndex = 3 then
     QDev.sql.add('and Enviado_DGII =1 and  Error_DGII=0 and AceptadoDGII = 0');
...
procedure TfrmConsDev.DBGrid1DrawColumnCell(Sender: TObject;
...
procedure TfrmConsDev.btnEnviarDGIIClick(Sender: TObject);
var
...
begin
  ...
  // ============================================================
  if not QDevAceptadoDGII.IsNull then
  begin
    if QDevEnviado_DGII.Value then
    begin
      if QDevAceptadoDGII.Value then
      begin
        ShowMessage('Esta devolución ya fue enviada y ACEPTADA por la DGII.');
        Exit; // Salimos del procedimiento para no reenviar
      end
      else if QDevError_DGII.Value then
      begin
        // Verificar permiso para reenviar rechazadas
        if VarIsNull(dm.usu_reenvia_dgii) or not dm.usu_reenvia_dgii then
        begin
          ShowMessage('Esta devolución ya fue enviada y RECHAZADA por la DGII.');
          Exit; // Salimos del procedimiento para no reenviar
        end
        else
        begin
          // >>> CONFIRMACIÓN <<<
          if MessageDlg(
               'Esta devolución fue RECHAZADA por la DGII.' + sLineBreak +
          ...
          dm.Query1.SQL.Clear;
          dm.Query1.SQL.Add(
            'SELECT dbo.fn_obtenerSecuenciaDGI(:emp, :tipo) AS NuevaSecuencia;'
          );
          dm.Query1.Parameters.ParamByName('emp').Value  := QDevEMP_CODIGO.Value;
          dm.Query1.Parameters.ParamByName('tipo').Value := 34; // Tipo eCF nota de crédito (devolución)
          ...
          dm.Query1.Close;
          dm.Query1.SQL.Clear;
          dm.Query1.SQL.Add('UPDATE Devolucion SET eNCF = :ncfNuevo, Enviado_DGII = 1, AceptadoDGII = 0, Error_DGII = 0 ');
          dm.Query1.SQL.Add('WHERE emp_codigo = :emp AND dev_numero = :num AND suc_codigo = :suc');
  ...
  dm.Query1.Parameters.ParamByName('emp').Value   := QDevEMP_CODIGO.Value;
  dm.Query1.Parameters.ParamByName('suc').Value   := QDevsuc_codigo.Value;
  dm.Query1.Parameters.ParamByName('num').Value   := QDevFAC_NUMERO.Value;
  dm.Query1.Parameters.ParamByName('forma').Value := QDevFAC_FORMA.Value;
  dm.Query1.Parameters.ParamByName('tfa').Value   := QDevTFA_CODIGO.Value;
  dm.Query1.Open;
  ...
  begin
    ...
    dm.Query1.Parameters.ParamByName('emp').Value := QDevEMP_CODIGO.Value;
    dm.Query1.Parameters.ParamByName('suc').Value := QDevsuc_codigo.Value;
    dm.Query1.Parameters.ParamByName('num').Value := QDevticket_numero.Value;
    dm.Query1.Parameters.ParamByName('usu').Value := QDevticket_cajero.Value;  // campo real del devolución
    dm.Query1.Parameters.ParamByName('caja').Value := QDevticket_caja.Value;      // idem
     dm.Query1.Open;
    Encontrado := not dm.Query1.IsEmpty;
  end;
  if not Encontrado then
  begin
    ShowMessage('No se encontró la factura asociada (Facturas ni Montos_Ticket). No se enviará la devolución a la DGII.');
    Exit;
  end;
    ...
    CausaCodigo := StrToIntDef(Trim(QDevmotivo_dgi.AsString), 0); // AJUSTA nombre si es otro campo

    case CausaCodigo of
      1: // Anulación total
        begin
          if Abs(QDevDEV_TOTAL.Value - TotalFactura) > 0.07 then
          begin
            ShowMessage(
              'Para la causa "1 - Anulación total", el monto de la devolución (' +
              CurrToStr(QDevDEV_TOTAL.Value) +
      ...
      3: // Corrección montos
        begin
          if (QDevDEV_TOTAL.Value <= 0) or
             (QDevDEV_TOTAL.Value > TotalFactura) then
          begin
            ShowMessage(
      ...
      2: // Corrección texto
        begin
          if Abs(QDevDEV_TOTAL.Value) > 0.01 then
          begin
            ShowMessage(
    ...
    try
      if (not QDevticket_numero.IsNull) and (Trim(QDevticket_numero.AsString) <> '') then
      begin
        Servicio := CoFacturaElectronicaService.Create;
        resultado := Servicio.EnviarDevolucionPOS(
                        IntToStr(QDevEMP_CODIGO.Value),
                        IntToStr(QDevsuc_codigo.Value),
                        '',
                        IntToStr(QDevDEV_NUMERO.Value),
                        QDevemp_rnc.Value,
                        QDeveNCF.Value,
                        QDevcli_rnc.Value,
                        '',
      ...
      begin
        ShowMessage('Devolución enviada correctamente.');
      end;
    finally
      Servicio := nil;
    end;
...
procedure TfrmConsDev.FrmPopupDGIIQDGIIParametersParamByNamecaja1Click(
  Sender: TObject);
begin
Application.CreateForm(TFrmPopupDGII, FrmPopupDGII);
try
  FrmPopupDGII.QDGII.Close;
  FrmPopupDGII.QDGII.Parameters.ParamByName('tipo').Value := 'DEVOLUCION';
  FrmPopupDGII.QDGII.Parameters.ParamByName('emp_codigo').Value := QDevEMP_CODIGO.Value;
  FrmPopupDGII.QDGII.Parameters.ParamByName('fac_numero').Value := QDevDEV_NUMERO.Value;
